...
4. Delegating complex fixes to task-executor-agent when needed
5. Checking for merge conflicts before re-review
6. Invoking conflict-resolver-agent when needed
7. Re-invoking pr-review-agent after fixes
8. Escalating to human after max iterations
9. Logging all state transitions for auditability
...
if [ "$MERGEABLE" = "CONFLICTING" ]; then
  ...
  # Use Task tool: subagent_type="ghpmplus:conflict-resolver"
...
```markdown
Use the Task tool with subagent_type="ghpmplus:pr-review" to:
...
**Status:** CHANGES_REQUESTED -> APPLYING_FIXES
...
Applying fixes for review feedback...
...
2. **For each piece of feedback**, determine if it can be fixed inline:
   - **Simple fixes** (naming, formatting, small logic changes, missing imports): Apply directly with Edit tool
   - **Complex fixes** (architectural changes, new test files, significant refactoring): These require more context

3. **Read the affected files** to understand context before editing
...
- Do not introduce new functionality beyond what the feedback requests
- Run any available linters/tests after applying fixes

#### Step 3.5: Verify Fixes Pushed
...
gh issue comment "$TASK_NUMBER" --body "PR #$PR_NUMBER has been escalated for human review after $MAX_ITERATIONS unsuccessful review iterations."
```
...
**Iteration:** N/MAX
**Status:** <PENDING_REVIEW|APPLYING_FIXES|ESCALATED|COMPLETE>
...
- Feedback properly applied via Edit/Write tools or escalated
