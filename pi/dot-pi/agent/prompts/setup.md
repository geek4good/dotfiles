...
- **show_plan** (built-in Pi tool) — TUI plan review with approve/decline gate
- **show_report** (built-in Pi tool) — TUI completion report viewer
- Commands: `/setup`, `/worktree`, `/save`, `/stable`, `/compact`, `/restore`, `/agent-memory`
- Skills: `agent-memory`, `dex`
...
4. **Plan-via-TUI**: when plan mode is active, or a plan file has just been written, use `show_plan` to present the plan in the terminal for approval before proceeding.
5. **Completion-via-TUI**: when work wraps, use `show_report` to present a completion summary with diffs in the terminal.
6. **Compact/restore loop**: new sessions begin with `/restore` if `.context/session-state.json` exists; long sessions trigger `/compact` via the memory-cycle hook.

### Bootstrap
If `agent-memory` is not on PATH, run `/setup` — it installs what's missing.
...
def main():
    ...
    print("  /agent-plan <topic>  — interactive plan builder")
